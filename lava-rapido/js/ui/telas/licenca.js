 ...
 * lido em voz alta ou colado num WhatsApp por alguém em pé, no pátio. Ele é
  ...
  async function desenhar() {
    ...
    const falar = cartao([
      ...
      temWhatsApp()
        ...
        : null,
      temEmail()
        ? botao('✉️ Enviar e-mail', () => { window.location.href = linkDeEmail('Licença do Lava-Rápido Lite', mensagem); }, { tipo: temWhatsApp() ? 'secundario' : 'principal', classe: 'largo' })
        : null,
    ...
    const seletor = el('input', {
      type: 'file',
      accept: '.lava,application/json,text/plain',
      classe: 'escondido',
    ...
    const ativar = cartao([
      ...
      el('p', { classe: 'observacao', texto: 'Quando receber o arquivo de licença (.lava), toque abaixo e escolha o arquivo. Funciona sem internet.' }),
    ...
    const detalhes = situacao.licenca
      ? cartao([
        ...
        botao('Remover licença deste aparelho', async () => {
          if (!await contexto.autorizar('Remover licença')) return;
          if (!await confirmar({
            titulo: 'Remover a licença?',
            texto: 'O aplicativo volta ao estado de avaliação neste aparelho. Use isto ao trocar de celular, guardando o arquivo .lava para importar no novo.',
            acao: 'Remover',
            perigo: true,
          })) return;
...
/**
 * O cartão que substitui o atendimento quando a licença não permite registrar.
 *
 * Aparece no lugar do primeiro passo da nova lavagem — e só ali. O resto do
 * aplicativo continua aberto, porque o movimento é do dono.
 */
export function cartaoDeBloqueio(contexto, situacao) {
  return cartao([
    el('p', { classe: 'comprovante-marca', texto: '🔒' }),
    el('h2', { classe: 'passo-titulo', texto: situacao.estado === ESTADOS.VENCIDA ? 'Licença vencida' : 'Avaliação encerrada' }),
    el('p', { classe: 'observacao', texto: 'Para registrar novas lavagens, ative a licença. O que já está no pátio pode ser finalizado e recebido, e o histórico, o caixa e o backup continuam liberados.' }),
    botao('Ver licença e falar com o desenvolvedor', () => contexto.ir('licenca'), { tipo: 'principal', classe: 'gigante' }),
