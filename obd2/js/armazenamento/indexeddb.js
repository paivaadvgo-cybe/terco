 ...
 * Driver IndexedDB.
 *
 * É o banco de verdade: sobrevive a fechar o aplicativo e a reiniciar o
 * aparelho. Aqui isso importa mais que no comum, porque uma viagem de meia hora
 * gravada a cada segundo são quase dois mil registros — `localStorage` é
 * síncrono (travaria a tela a cada amostra, com o carro andando), só guarda
 * texto e acaba em poucos megabytes.
 *
 * Este arquivo não conhece viagem, PID nem falha. Ele conhece coleções, chaves
 * e índices, e é essa fronteira que deixa os testes rodarem no Node contra o
 * driver em memória, sem navegador nenhum.
 */
...
export async function criarDriverIndexedDB(nome = NOME, versao = VERSAO) {
  ...
  return {
     ...
     *
     * É o caminho normal da gravação de viagem: as amostras são acumuladas e
     * descarregadas em lote. Uma transação por amostra, a cada segundo, com o
     * aplicativo em primeiro plano desenhando ponteiros, é gasto de bateria e
     * de tempo por nada — e se cair no meio, o lote inteiro é descartado junto,
     * em vez de deixar meia gravação no banco.
     */
