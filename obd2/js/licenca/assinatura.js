...
/**
 * O nome deste aplicativo dentro do envelope.
 *
 * É o que impede uma licença do Lava-Rápido de ativar o Painel OBD-II e
 * vice-versa. Os dois compartilham o par de chaves e o gerador — uma chave
 * privada só, num lugar só —, e é este campo, assinado junto com o resto, que
 * separa um do outro. Sem ele, um cliente de um aplicativo teria o outro de
 * graça.
 */
export const APP = 'PainelOBD2';

/** A mesma chave pública do Lava-Rápido: um par de chaves serve os dois. */
...
export async function envelopeConfere(envelope, chavePublica = CHAVE_PUBLICA) {
  try {
    if (!envelope || envelope.app !== APP || envelope.tipo !== 'licenca') return false;
...
export async function assinarLicenca(licenca, chavePrivadaJwk) {
  ...
  return { app: APP, tipo: 'licenca', versao: 1, licenca, sig: paraBase64(assinatura) };
}

/** O arquivo `.obd2` é o envelope em base64 — texto curto, que cabe num WhatsApp. */
