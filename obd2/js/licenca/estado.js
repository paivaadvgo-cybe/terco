 ...
 *
 * O combinado é o mesmo do Lava-Rápido, e é de propósito que seja o mesmo:
 *
 *     dia 0 ── 7 ────────────────── 30 ──▶
 *     [ silêncio ][ aviso em toda tela ][ conexão ao carro bloqueada ]
 *
 * Os sete primeiros dias são calados de propósito: quem acabou de instalar
 * está decidindo se o aplicativo serve, e um aviso de cobrança na primeira
 * viagem responde a pergunta pelo lado errado. Do sétimo ao trigésimo o aviso
 * fica em todas as telas, sem botão de fechar — é o período em que a pessoa
 * precisa lembrar de procurar o desenvolvedor, e um aviso que se fecha vira um
 * aviso que nunca foi lido.
 *
 * Passados os trinta dias, **só a conexão a um adaptador de verdade é
 * bloqueada**. O que foi gravado continua na tela: as viagens, os gráficos, os
 * máximos, a exportação e a importação. O carro simulado também continua, para
 * que o aplicativo possa ser mostrado a quem ainda vai decidir. Trancar alguém
 * do lado de fora das próprias viagens seria usar o dado dele como refém — e o
 * aplicativo que promete que nada sai do aparelho não pode ser o que te impede
 * de olhar o que é seu.
 *
...
/**
 * Quem pode conectar num adaptador de verdade.
 *
 * O resto do aplicativo nunca é bloqueado — nem o carro simulado, que é como
 * se mostra o aplicativo a quem ainda não decidiu.
 */
export function permiteConectar(estado) {
...
function montar(estado, { agora, inicioDaAvaliacao, licenca }) {
  ...
  const situacao = {
    ...
    licenca,
    permiteConectar: permiteConectar(estado),
  };
...
function severidadeDe({ estado, diasRestantes, avisar }) {
  ...
  if (!permiteConectar(estado)) return 'perigo';
