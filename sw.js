...
self.addEventListener('activate', evento => {
  evento.waitUntil(
    caches.keys()
      // Só apagamos caches nossos. `caches.keys()` enxerga o domínio inteiro,
      // e outros aplicativos publicados no mesmo endereço têm os seus — apagar
      // tudo derrubaria o modo offline deles.
      .then(chaves => Promise.all(
        chaves.filter(k => k.startsWith('airdrums-') && k !== CACHE).map(k => caches.delete(k))
      ))
...
/* Caminho onde este aplicativo vive, deduzido da própria localização do
   worker. Serve para não encostar em nada que esteja fora daqui. */
const BASE = new URL('./', self.location.href).pathname;

/** Este endereço pertence ao AirDrums? */
function meu(caminho){
  if(caminho === BASE) return true;                       // a própria pasta
  const resto = caminho.startsWith(BASE) ? caminho.slice(BASE.length) : null;
  // Só os arquivos do aplicativo, e nada em subpastas: outro aplicativo
  // publicado no mesmo domínio precisa continuar sendo servido normalmente,
  // com o service worker dele, e nunca com o cache ou o manifesto deste.
  return resto !== null && !resto.includes('/') &&
         ['', 'index.html', 'sw.js', 'manifest.json', 'icone.svg'].includes(resto);
}
...
self.addEventListener('fetch', evento => {
  ...
  const proprio = url.origin === self.location.origin && meu(url.pathname);

  if(!externo && !proprio) return;    // não é nosso: segue o fluxo normal
  ...
  /* --- MediaPipe e arquivos do app: cache primeiro ---------------------- */
  evento.respondWith(
