  ...
  evento.waitUntil(
    caches.keys().then(function(chaves){
      return Promise.all(chaves.map(function(chave){
        /* Só as versões antigas DESTE aplicativo. `caches.keys()` enxerga o
           domínio inteiro, e há outro aplicativo publicado ao lado: apagar
           tudo o que não é nosso derrubaria o modo offline dele. */
        if (chave !== CACHE && chave.indexOf("santo-terco") === 0) return caches.delete(chave);
        return null;
      }));
